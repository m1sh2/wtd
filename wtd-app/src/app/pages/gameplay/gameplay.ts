...
export class Gameplay {
  ...
  getBase() {
    ...
    const attrs = [];
    jQuery.each(jQuery('#wpns')[0].attributes, function() {
    ...
    self.BASE.wpns.forEach(function(pos) {
      let wpnPos = jQuery(document.createElement('div'));
      wpnPos
        .addClass('wpn-pos')
        .css({
          left: pos.x - 20 + 'px'
  ...
  drBullet(o) {
    ...
    switch(o.type) {
      case 'bullet':
        ...
        self.ctx.beginPath();
        self.ctx.moveTo(o.x, o.y);
        self.ctx.lineTo(tX, tY);
        self.ctx.fillStyle = 'rgba(0, 0, 0, 0)';
        self.ctx.strokeStyle = 'rgba(' + o.color + ', ' + o.opacity + ')';
        self.ctx.lineWidth = o.bullet.width;
        self.ctx.stroke();
        self.ctx.closePath();
        break;
  ...
  drEnemy(o) {
    ...
    for (let k = 0;k < self.wpnsCurrent.length;k ++ ) {
      ...
      if (len < r
        ...
        if (o.life <= 0) {
          ...
          // delete ENY[o.id];
          self.ENYS = self.ENYS.filter(function(obj) {
           return obj !== o.id
          });
    ...
    // // self.ctx.drawImage(img, o.x, o.y);
    // self.ctx.drawImage(img, 0, 0, 32, 32, o.x, o.y, 16, 16);
    
    self.ctx.fillStyle = o.color;
    self.ctx.arc(o.x, o.y, o.w, 0, 2 * Math.PI);
    self.ctx.fill();
  ...
  drWpn(o) {
    ...
    // // self.ctx.drawImage(img, o.x, o.y);
    // self.ctx.drawImage(img, 0, 0, 32, 32, 0, 0, 32, 32);
